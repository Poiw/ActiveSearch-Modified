...
void write_correspondences(std::ofstream& ofs,
                           const std::vector< float >& c2D,
                           const std::vector< float >& c3D) {
  for (size_t i = 0, len = c2D.size() / 2; i < len; ++i) {
    ofs << c2D[i * 2] << ' ';
    ofs << -c2D[i * 2 + 1] << ' ';
    ofs << c3D[i * 3] << ' ';
    ofs << c3D[i * 3 + 1] << ' ';
    ofs << c3D[i * 3 + 2] << ' ' << std::endl;
  }
}
...
//         std::cout << " "  << corr_3D_to_2D.size() << std::endl;
    ...
    // output the solution:
    std::cout << "#### found correspondences ####" << std::endl;
    ...
    //ofs << nb_corr << " " << vw_time << " " << corr_time << " " << all_timer.GetElapsedTime() << std::endl;
