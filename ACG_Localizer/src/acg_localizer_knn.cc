...
int main (int argc, char **argv)
{
  ...
  ANNcoord *ann_query_descriptor = new ANNcoord[sift_dim];
...

  {
    ...
    if( method == 0 || method == 3 )
      all_descriptors_float.resize(sift_dim*nb_descriptors);
    else
    ...
    for( uint32_t i=0; i<nb_descriptors; ++i )
    {
      if( method == 1 || method == 2 )
		tree_descriptors[i] = new ANNcoord[sift_dim];
      float length = 0.0f;
      for( uint32_t j=0; j<sift_dim; ++j )
      {
		...
		if( desc_mode == 0 || desc_mode == 2 )
		{
		  ...
		  if( method == 0 || method == 3 )
			all_descriptors_float[sift_dim*i+j] = float(entry_char);
		  else
		  {
		...
		else
		{
		  ...
		  if( method == 0 || method == 3 )
			all_descriptors_float[sift_dim*i+j] = entry_float;
		  else
		  {
  ...
  {
    vw_handler.set_branching( sift_dim );
  ...
  else
  {
    ...
    // create the kd-tree
    kd_tree = new ANNkd_tree(tree_descriptors, nb_tree_descriptors, sift_dim);
