...
uint32_t compute_medoid( std::vector< unsigned char > &desc )
{
  ...
  for( uint32_t i=0; i<nb_desc; i+=sift_dim )
  {
    double cur_dist = 0.0;
    for( uint32_t j=0; j<nb_desc; j+=sift_dim )
    {
      double dist = 0.0;
...
{
  mean.resize( sift_dim, 0 );
  uint32_t N = (uint32_t) desc.size() / sift_dim;
  for( uint32_t i=0; i<N; ++i )
  {
    uint32_t index = i*sift_dim;
...
int main (int argc, char **argv)
{
  ...
  for (int i = 0; i < sift_dim; ++i) {
    std::cout << (uint32_t)feature_infos[233].descriptors[i] << std::endl;
  }
  while(1);
  */
  ...
  std::vector< unsigned char > tmp_descriptors( sift_dim * batch_size );
  ...
  for( uint32_t i=0; i<nb_points; ++i )
  {
    ...
    {
      // copy the descriptor
      index1 = selected_desc * sift_dim;
      index2 = uint64_t(j)*uint64_t(sift_dim);
      for( uint64_t k=0; k<sift_dim; ++k )
      ...
      if( (selected_desc == batch_size) || (offset+selected_desc == total_nb_descriptors) || (total_offset+selected_desc == total_nb_descriptors ) )
      {
		...
		for( uint32_t l=0; l<selected_desc; ++l )
		{
		  if( cluster_assignments_[l] > nb_cluster )
		  {
			...
			for( uint32_t ll=0; ll<sift_dim; ++ll )
			  std::cout << " " << int(tmp_descriptors[sift_dim*l+ll]);
			std::cout << std::endl;
      cluster_assignments_[l] = 0;
		  }
...
// 		std::cout << offset << " / " << total_nb_descriptors << std::endl;
  ...
  for( uint32_t i=0; i<nb_points; ++i )
  {
    ...
    if ( mode == 0 )
    {
      ...
      for( std::set< size_t >::iterator it = activated_visual_words.begin(); it != activated_visual_words.end(); ++it )
      {
		...
		for( size_t j=0; j<nb_desc_i; ++j )
		{
		  ...
		  {
			for( uint32_t k=uint32_t(j)*sift_dim; k < uint32_t(j)*sift_dim+sift_dim; ++k )
		...
		uint32_t desc_id = uint32_t( descriptors.size() ) / sift_dim;

		// add the new descriptor to the set of all used descriptors
		for( uint32_t k=sift_dim*med_id; k<(sift_dim*med_id + sift_dim ); ++k )
    ...
    else if( mode == 4 )
    {
      ...
      for( std::set< size_t >::iterator it = activated_visual_words.begin(); it != activated_visual_words.end(); ++it )
      {
		...
		uint32_t desc_id = uint64_t( descriptors_float.size() ) / sift_dim;
    ...
    else if( mode == 5 )
    {
      ...
      for( std::set< size_t >::iterator it = activated_visual_words.begin(); it != activated_visual_words.end(); ++it )
      {
		for( size_t j=0; j<nb_desc_i; ++j )
		{
		  if( *it == descriptor_2_vw_assignments[offset+j] )
		  {
			uint32_t desc_id = uint64_t( descriptors.size() ) / sift_dim;
			for( uint32_t k=uint32_t(j)*sift_dim; k < uint32_t(j)*sift_dim+sift_dim; ++k )
			  descriptors.push_back(feature_infos[i].descriptors[k]);
			vw_point_descriptor_idx[ *it ].push_back(std::make_pair( i, desc_id ) );
		  }
    ...
    else if( mode == 6 )
    {
      ...
      for( std::set< size_t >::iterator it = activated_visual_words.begin(); it != activated_visual_words.end(); ++it )
      {
		...
		std::vector< unsigned char > integer_mean( sift_dim, 0 );
  ...
  uint32_t nb_non_empty_vw = 0;
  {
    ...
    std::cout << " # computed medoid descriptors: " << descriptors.size() / sift_dim << std::endl;
    std::cout << " # computed mean descriptors: " << descriptors_float.size() / sift_dim << std::endl;
  ...
  std::cout << "-> saving the descriptor-to-visual word assignments to " << desc_output << std::endl;
  {
	...
	if( mode == 0 || mode == 3 || mode == 5 || mode == 6 )
	  nb_descriptors = uint32_t( descriptors.size()/sift_dim );
	else if( mode == 1 || mode == 2 || mode == 4 )
	  nb_descriptors = uint32_t( descriptors_float.size()/sift_dim );
  ...
  if( mode == 0 || mode == 3 || mode == 5 || mode == 6 )
    std::cout << " " << uint32_t( descriptors.size()/sift_dim ) << " descriptors -> " << uint32_t( descriptors.size() ) << " unsigned chars -> " << uint32_t( descriptors.size() ) / ( uint32_t(1024) * uint32_t(1024) ) * sizeof( unsigned char ) << " MB " << std::endl;
  else if( mode == 1 || mode == 2 || mode == 4  )
    std::cout << " " << uint32_t( descriptors_float.size()/sift_dim ) << " descriptors -> " << uint32_t( descriptors_float.size() ) << " floats -> " << uint32_t( descriptors_float.size() ) / ( uint32_t(1024) * uint32_t(1024) ) * sizeof( float ) << " MB " << std::endl;
